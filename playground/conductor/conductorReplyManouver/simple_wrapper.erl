...
%% Public API - now returns {Reply, Context} tuples
-spec call(pid() | atom(), term(), reference() | undefined, atom()) -> {any(), reference()}.
...
%% Worker process does the actual communication and returns {Reply, Context}
worker_process(To, Msg, Context, Module, From, ToModule) ->
    ...
    try
        ...
        io:format("[~p ms] [WRAPPER_WORKER] COMPLETE: Sending reply {~p, ~p} (took ~p ms)~n", 
                 [EndTime, Reply, Context, Duration]),
        
        %% Send monitor notification for response
        monitor ! {ToModule, Module, Reply, Context},
        
        %% Send reply as {Reply, Context} tuple - FORWARD CONTEXT PROPAGATION
        gen_server:reply(From, {Reply, Context})
        
    catch
        Error:Reason:Stacktrace ->
            io:format("[WRAPPER_WORKER] ERROR: ~p:~p~n~p~n", [Error, Reason, Stacktrace]),
            gen_server:reply(From, {{error, processing_failed}, Context})
    end.
