...
%%── Inject BOTH context handlers for record-based approach
inject_both_context_handlers_record(Forms) ->
    ModuleName = extract_module_name(Forms),
    lists:map(fun
        ({function, Line, handle_call, 3, Clauses}) ->
            io:format("PARSE_TRANSFORM: Injecting BOTH context handlers into handle_call/3 (record)~n"),
            
            %% NEW: Atomic with_context handler - highest priority
            ...
            %% OLD: Simple context handler - for receiving context back (especially clients)
            ContextOnlyClause = {clause, Line,
            ...
            {function, Line, handle_call, 3, [WithContextClause, ContextOnlyClause | Clauses]};
        (Other) ->
            Other
    end, Forms).
...
%% Detect if a clause is an injected handler (more robust detection)
...
%%── Inject BOTH context handlers for map-based approach
inject_both_context_handlers_map(Forms) ->
    ModuleName = extract_module_name(Forms),  %% FIX: Extract module name here!
    lists:map(fun
        ({function, Line, handle_call, 3, Clauses}) ->
            io:format("PARSE_TRANSFORM: Injecting BOTH context handlers into handle_call/3 (map)~n"),
            ...
            WithContextClause = {clause, Line,
               ...
               [ {match,Line,{var,Line,'ReceiveTime'},
                 ...
                 {call,Line,{remote,Line,{atom,Line,io},{atom,Line,format}},
                     [{string,Line,"[~p ms] [~p] RECEIVED with_context: ctx=~p, msg=~p~n"},
                      {cons,Line,{var,Line,'ReceiveTime'},
                          {cons,Line,{atom,Line,ModuleName},  %% FIX: Use ModuleName instead of '?MODULE'
                              {cons,Line,{var,Line,'Context'},
                                  {cons,Line,{var,Line,'Msg'},{nil,Line}}}}}]},
                 {match,Line,{var,Line,'StateWithContext'},
                     ...
                     [{string,Line,"[~p ms] [~p] Context ~p injected, delegating message ~p~n"},
                      {cons,Line,{var,Line,'ReceiveTime'},
                          {cons,Line,{atom,Line,ModuleName},  %% FIX: Use ModuleName instead of '?MODULE'
                              {cons,Line,{var,Line,'Context'},
                                  {cons,Line,{var,Line,'Msg'},{nil,Line}}}}}]},
                 {call,Line,{remote,Line,{atom,Line,ModuleName},{atom,Line,handle_call}}, %% FIX: Use ModuleName instead of '?MODULE'
                     [{var,Line,'Msg'},{var,Line,'From'},{var,Line,'StateWithContext'}]}
               ]},

            %% OLD: Simple context handler - for receiving context back 
            ContextOnlyClause = {clause, Line,
               [ {tuple,Line,[{atom,Line,context},{var,Line,'Context'}]},
                 {var,Line,'_From'},
                 {var,Line,'State'}
