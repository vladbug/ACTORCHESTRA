...
worker_process(To, Msg, Context, Module, From, ToModule) ->
    ...
    try
        ...
        spawn(fun() ->
            ...
            io:format("[~p ms] [WRAPPER_WORKERVLADYS] Context ~p back-propagated to caller ~p~n", 
                     [erlang:system_time(millisecond), Context, CallingPid])
        end),
        ...
        io:format("[~p ms] [WRAPPER_WORKERVLADYS] Reply sent to caller ~p~n", 
                 [erlang:system_time(millisecond), From])
    catch
