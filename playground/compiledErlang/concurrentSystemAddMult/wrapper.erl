    ...
    case queue:out(Queue) of
        ...
        {{value, #operation{from = From, msg = Msg, context = Context}}, _} ->
            try
                ...
                % To ensure the context is set in the original caller - DISABLED FOR NOW
                %{CallingPid, _Ref} = From,
                %io:format("[wrapper] sending context ~p back to calling process ~p~n", [Context, CallingPid]),
                %ok = gen_server:call(CallingPid, {context, Context}), 
                {CallingPid, _Ref} = From,
                %% CRITICAL: Only send context back to processes that can handle it
                %% Check if the calling process is a gen_server (not a test process)
                case is_process_alive(CallingPid) of
                    true ->
                        case catch gen_server:call(CallingPid, {context, Context}, 100) of
                            ok ->
                                io:format("[wrapper] successfully sent context ~p back to calling process ~p~n", [Context, CallingPid]);
                            {'EXIT', _} ->
                                io:format("[wrapper] calling process ~p cannot handle context, skipping~n", [CallingPid]);
                            {timeout, _} ->
                                io:format("[wrapper] timeout sending context to ~p, skipping~n", [CallingPid])
                        end;
                    false ->
                        io:format("[wrapper] calling process ~p is not alive, skipping context~n", [CallingPid])
                end,
                io:format("[wrapper] about to send reply ~p to ~p~n", [Reply, From]),
                gen_server:reply(From, Reply),
                io:format("[wrapper] reply sent successfully~n")
            catch
                Error:Reason ->
