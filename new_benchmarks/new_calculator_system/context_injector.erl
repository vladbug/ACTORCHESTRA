    ...
    lists:map(fun
        ({function, Line, handle_call, 3, Clauses}) ->
            ...
            NewClauses = case is_client_module(ModuleName) of
                true ->
                    %% For client modules, add get_current_context handler
                    GetContextClause = {clause, Line,
                       [{atom, Line, get_current_context}, {var, Line, '_From'}, {var, Line, 'State'}],
                       [],
                       [{tuple, Line, [
                           {atom, Line, reply},
                           {tuple, Line, [
...
transform_client_body_record([], Acc) ->
    lists:reverse(Acc);
transform_client_body_record([Expr | Rest], Acc) ->
    case Expr of
        ...
        {match, _MLine, {var, VLine, VarName}, 
         {call, CLine, {remote, _RLine, {atom,_,gen_server}, {atom,_,call}}, [Target, Msg]}} ->
            %% Transform to tuple matching and direct return
            ...
            transform_client_body_record(Rest, [TupleMatch | Acc]);
        
        %% Pattern: {reply, SomeVar, State} - Transform to include context update
        {tuple, Line, [{atom, _, reply}, ReplyVar, {var, _, 'State'}]} ->
            %% Replace with: {reply, ReplyVar, maps:put(context, NewContext, State)}
            ...
            transform_client_body_record(Rest, [NewStateTuple | Acc]);
        
        %% Other expressions - recursively transform
        _ ->
            NewExpr = transform_expr_record(Expr, client),
            transform_client_body_record(Rest, [NewExpr | Acc])
    end.
...
transform_client_body_map([], Acc) ->
    lists:reverse(Acc);
transform_client_body_map([Expr | Rest], Acc) ->
    case Expr of
        %% Pattern: Variable = gen_server:call(...)
        {match, _MLine, {var, VLine, VarName}, 
         {call, CLine, {remote, _RLine, {atom,_,gen_server}, {atom,_,call}}, [Target, Msg]}} ->
            ...
            TupleMatch = {match, VLine,
                         ...
                         {call, CLine, {remote, CLine, {atom,CLine,simple_wrapper}, {atom,CLine,call}},
                          [Target, Msg, {var, CLine, 'Context'}, {atom, CLine, client}]}},
            transform_client_body_map(Rest, [TupleMatch | Acc]);
        ...
        {tuple, Line, [{atom, _, reply}, ReplyVar, {var, _, 'State'}]} ->
            ...
            NewStateTuple = {tuple, Line, [
                ...
                ReplyVar,
                {call,Line,{remote,Line,{atom,Line,maps},{atom,Line,put}},
                 [{atom,Line,context},{var,Line,'NewContext'},{var,Line,'State'}]}
            ]},
            transform_client_body_map(Rest, [NewStateTuple | Acc]);
        
        %% Other expressions - recursively transform
        _ ->
            NewExpr = transform_expr_map(Expr, client),
            transform_client_body_map(Rest, [NewExpr | Acc])
    end.
