-module(manual_instrumented_chat_test).
...
run_simple_test() ->
    io:format("=== MANUAL INSTRUMENTED CHAT TEST - SINGLE MESSAGE ===~n"),
...
run_complex_test() ->
    io:format("=== MANUAL INSTRUMENTED CHAT TEST - MULTIPLE MESSAGES ===~n"),
    ...
    %% Complex workload - 50 messages (same as baseline)
    NumberOfMessages = 50,
    io:format("Sending ~p messages sequentially...~n", [NumberOfMessages]),
    
    StartTime = erlang:system_time(microsecond),
    
    %% Send messages sequentially (same pattern as baseline)
    Results = lists:map(fun(N) ->
        Message = io_lib:format("Test message number ~p from instrumented client", [N]),
    ...
    %% Results (same calculation as baseline)
...
run_session_test() ->
    io:format("=== MANUAL INSTRUMENTED CHAT TEST - COMPLETE SESSION ===~n"),
    ...
    NumberOfMessages = 20,  % Same as baseline
    ...
    %% Phase 2: Send messages (same pattern as baseline)
...
run_multi_client_test() ->
    io:format("=== MANUAL INSTRUMENTED CHAT TEST - MULTI-CLIENT SESSION ===~n"),
    ...
    %% Concurrent messaging (same pattern as baseline)
    StartTime = erlang:system_time(microsecond),
    ParentPid = self(),
    ...
    %% Analyze results (same calculation as baseline)
...
test_quick() ->
    io:format("Quick instrumented chat test:~n"),
    Time = run_simple_test(),
    io:format("~n=== RESULT: ~p microseconds ===~n", [Time]).

test_messages() ->
    io:format("Message batch instrumented chat test:~n"),
...
test_session() ->
    io:format("Complete session instrumented chat test:~n"),
...
test_multi() ->
    io:format("Multi-client instrumented chat test:~n"),
